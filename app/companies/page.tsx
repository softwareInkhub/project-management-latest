...
const CompaniesPage = () => {
  ...
  const insertMarkdown = (prefix: string, suffix: string = '', placeholder: string = 'text') => {
    const textarea = document.getElementById('company-description-editor') as HTMLTextAreaElement;
    if (!textarea) return;
    ...
    <AppLayout>
      ...
      <div className="w-full h-full px-3 sm:px-4 lg:px-8 py-3 sm:py-4 lg:py-4 overflow-x-hidden">
        ...
        {isModalOpen && (
          ...
          >
            <div 
              className="bg-white rounded-t-2xl lg:rounded-2xl shadow-2xl w-full lg:max-w-3xl h-[85vh] lg:h-auto lg:max-h-[95vh] flex flex-col"
                ...
                <div>
                  ...
                  {!isDescriptionPreview ? (
                    <textarea
                      ...
                      rows={5}
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg bg-white text-gray-900 focus:ring-2 focus:ring-blue-500 focus:border-transparent font-mono text-sm resize-none"
                      placeholder="Enter company description (Markdown supported)"
                    />
                  ) : (
                ...
                <div>
                  ...
                  {viewingCompany.description ? (
                    <div className="text-gray-900 bg-gray-50 px-4 py-3 rounded-lg border border-gray-200">
                      <MarkdownRenderer content={viewingCompany.description} />
                    </div>
                  ) : (
